...
# import py2PcdFilters class
from py2_pcd_filtering import py2PcdFilters 
...
class Points_extract():
    ...
    def __init__(self):
        ...
        self.t_listener = tf.Transformer(True, rospy.Duration(500))
        # self.tfBuffer = tf2_ros.Buffer(rospy.Time.from_sec(300))
    ...
    def callback(self, depth_image, boxes_detect):
        ...
        if boxes_detect.bounding_boxes != []:
            ...
            time_tf = rospy.Time(0) - depth_head.stamp
            # get the transforma te the time
            (trans,rot) = self.listener.lookupTransform("/map", "/frontright_fisheye", depth_head.stamp)
            ...
            #
            self.publish_point_cloud(depth_head, transform)

    def transformPCD(self,point_cloud, transform):
        """ 
    ...
    def publish_point_cloud(self, depth_head, transform):
        new_pcd_msg = []
        ...
        for pcd in self.point_clouds:
            ...
            points_c_msg.header.frame_id = 'map'
            ...
            self.transformPCD(points_c_msg, transform)
            new_pcd_msg.append(points_c_msg)
    ...
    def extract_object(self, depth_proceed, boxes_coord, trans):
        ...
        for boxes_points in boxes_coord:
            # start the timne for extraction
            ...
            # Filter the point cloud using the py2 filtering class
            local_point_clouds[index] = self.firstPcdFilteringLayer(local_point_clouds[index])
            # Stop the timing for extraction
            end_time = time.time()
            # Calculate the time for extraction
            time_elapsed = end_time - start_time
            print "Time elapsed for extraction:", round(time_elapsed,3), "seconds."
    ...
    def firstPcdFilteringLayer(self, point_clouds):
        # Filter the point cloud using the py2 filtering class
        # create an object from the class
        pcdFilt = py2PcdFilters()
        # save the point cloud in the object
        pcdFilt.pcd_array = pcdFilt.ptsArr2numpyArr(point_clouds)  
        # execute the DBSCAN function from the class on data
        pcdFilt.DBSCAN(pcdFilt.pcd_array)
        densiest_points = pcdFilt.calc2dClustMetrics()
        # calculate 2D cluster surface and density
        print ' The 2 vals are :', len(densiest_points), len(pcdFilt.pcd_array)
        dense_pop_perc = int(len(densiest_points)) / int(len(pcdFilt.pcd_array))
        # print the result of the percentage
        print 'Cluster size :', dense_pop_perc
        # define the new local point cloud variable
        new_point_clouds = []
        # Check if the cluster is big enough to be considered as a cluster
        # if dense_pop_perc < 0.4:
        #     rospy.loginfo('The cluster is not big enough to be considered for the classification')
        #     # return the new point cloud
        #     return new_point_clouds
        # process a second layer of filtering
        # Apply second DBSCAN on the filtered data
        pcdFilt.DBSCAN(densiest_points)
        # Get the biggest cluster from the 3D segmentation
        new_obj_cluster = pcdFilt.getBiggestSegment(pcdFilt.main_segment)
        new_point_clouds = pcdFilt.numpyArr2ptsArr(new_obj_cluster)
        # return the new pcd in pts list
        return new_point_clouds
